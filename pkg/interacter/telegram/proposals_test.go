...
func TestProposalActiveInvalidInvocation(t *testing.T) {
	httpmock.Activate()
	...
	httpmock.RegisterMatcherResponder(
		...
		types.TelegramResponseHasText("Usage: /proposals [chain]"),
	...
	err = interacter.TelegramBot.Trigger("/proposals", ctx)
	require.NoError(t, err)
...
//nolint:paralleltest // disabled
func TestProposalActiveErrorFetchingChains(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterMatcherResponder(
		"POST",
		"https://api.telegram.org/botxxx:yyy/sendMessage",
		types.TelegramResponseHasText("❌ Error getting proposals: custom error"),
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("telegram-send-message-ok.json")))
	...
	mock.ExpectQuery("SELECT chain FROM chain_binds").
		WillReturnRows(sqlmock.NewRows([]string{"chain"}).AddRow("chain").AddRow("chain2"))
...
//nolint:paralleltest // disabled
func TestProposalActiveErrorFetchingExplorers(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	mock.ExpectQuery("SELECT name, pretty_name, base_denom, bech32_validator_prefix FROM chains").
		WillReturnRows(sqlmock.
			NewRows([]string{"name", "pretty_name", "base_denom", "bech32_validator_prefix FROM chains"}).
...
//nolint:paralleltest // disabled
func TestProposalActiveQueryError(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterMatcherResponder(
		"POST",
		"https://api.telegram.org/botxxx:yyy/sendMessage",
		types.TelegramResponseHasBytes(assets.GetBytesOrPanic("responses/proposals-active-error.html")),
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("telegram-send-message-ok.json")))

	httpmock.RegisterResponder(
		"GET",
		"https://example.com/cosmos/gov/v1/proposals?pagination.limit=1000&proposal_status=PROPOSAL_STATUS_VOTING_PERIOD",
	...
	mock.ExpectQuery("SELECT chain, name, proposal_link_pattern, wallet_link_pattern, validator_link_pattern, main_link FROM explorers").
		WillReturnRows(sqlmock.
			...
			AddRow("chain1", "Ping", "https://example.com/proposals/%s", "", "", ""),
		)
...
//nolint:paralleltest // disabled
func TestProposalActiveOk(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterMatcherResponder(
		"POST",
		"https://api.telegram.org/botxxx:yyy/sendMessage",
		types.TelegramResponseHasBytes(assets.GetBytesOrPanic("responses/proposals-active-ok.html")),
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("telegram-send-message-ok.json")))

	httpmock.RegisterResponder(
		"GET",
		"https://example.com/cosmos/gov/v1/proposals?pagination.limit=1000&proposal_status=PROPOSAL_STATUS_VOTING_PERIOD",
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("proposals-active.json")))
