...
func TestTelegramLCDAddNotEnoughArgs(t *testing.T) {
	httpmock.Activate()
	...
	httpmock.RegisterMatcherResponder(
		...
		types.TelegramResponseHasText("Usage: /lcd_add &lt;chain name&gt; &lt;host&gt;"),
	...
	err = interacter.TelegramBot.Trigger("/lcd_add", ctx)
	require.NoError(t, err)
...
//nolint:paralleltest // disabled
func TestTelegramLCDAddErrorFindingChain(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	ctx := interacter.TelegramBot.NewContext(tele.Update{
		...
		Message: &tele.Message{
			Sender: &tele.User{Username: "testuser", ID: 1},
			Text:   "/lcd_add chain https://example.com",
			Chat:   &tele.Chat{ID: 2},
		},
	})
...
//nolint:paralleltest // disabled
func TestTelegramLCDAddErrorInserting(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterMatcherResponder(
		"POST",
		"https://api.telegram.org/botxxx:yyy/sendMessage",
		types.TelegramResponseHasText("Error inserting LCD host!"),
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("telegram-send-message-ok.json")))
	...
	mock.ExpectExec("INSERT INTO lcd").WillReturnError(errors.New("custom error"))
...
//nolint:paralleltest // disabled
func TestTelegramLCDAddOk(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	httpmock.RegisterMatcherResponder(
		"POST",
		"https://api.telegram.org/botxxx:yyy/sendMessage",
		types.TelegramResponseHasBytes(assets.GetBytesOrPanic("responses/lcd-add.html")),
		httpmock.NewBytesResponder(200, assets.GetBytesOrPanic("telegram-send-message-ok.json")))
	...
	mock.ExpectExec("INSERT INTO lcd").WillReturnResult(sqlmock.NewResult(1, 1))
